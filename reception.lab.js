...
function readiness(r){
  ...
  (r.files?.prescription?done:block).push("Receta mÃÂ©dica");
  const pc=photoCount(r); (pc>=6?done:block).push(`FotografÃÂ­as ${pc}/10`);
  ...
  (r.checks?.fabricationProposalApproved?done:block).push("AprobaciÃÂ³n tÃÂ©cnica");
  ...
  return{color:"green",label:"Listo para producciÃÂ³n",canSend:true,done,block,warn};
}
function inject(){
  ...
  if(nav&&!q('[data-section="recepcion-lab"]')){
    ...
    b.innerHTML='<span class="icon">R</span><span>RecepciÃÂ³n LAB</span>'; nav.appendChild(b);
  ...
  if(main&&(!q("#recepcion-lab")||!q("#recepcion-lab").children.length)){
    ...
    s.innerHTML<`<div class="reception-hero"><div><p class="eyebrow">ASTOR ÃÂ· RECEPCIÃÂN LAB</p><h3>Finalizar caso y enviar a producciÃÂ³n</h3><p>Revisa los requisitos antes de liberar el caso.</p></div><div id="receptionTraffic" class="reception-traffic red"><span></span><strong id="receptionLabel">Sin caso</strong></div></div>
    <div class="reception-case"><div><small>Paciente</small><strong id="rPatient">Ã¢ÂÂ</strong></div><div><small>SÃÂ³digo clÃÂ­nico</small><strong id="rClinical">Ã¢ÂÂ</strong></div><div><small>CÃÂ³digo producciÃÂ³n</small><strong id="rProduction">Ã¢ÂÂ</strong></div><div><small>Sede</small><strong id="rCity">Ã¢ÂÂ</strong></div></div>
...
function render(){
  ...
  t.className=`reception-traffic ${st.color}`;q("#receptionLabel").textContent=st.label;q("#rPatient").textContent=r?.fields?.fullName||"Ã¢ÂÂ";q("#rClinical").textContent=r?.code||"Ã¢ÂÂ";q("#rProduction").textContent=r?shortCode(r):"Ã¢ÂÂ";q("#rCity").textContent=r?.fields?.city||"Ã¢ÂÂ";q("#rDone").innerHTML=li(st.done,"Nada completado");q("#rBlock").innerHTML=li(st.block,"Sin bloqueos");q("#rWarn").innerHTML=li(st.warn,"Sin advertencias");q("#rSend").disabled=!st.canSend;loadFields(r);
}
async function send(){
  const r=rec();if(!r)return alert("Selecciona un caso.");persist();const st=readiness(r);
  if(!r.checks?.receptionFinalConfirmation)return alert("Confirma la revisiÃÂ³n final.");
  if(!st.canSend)return alert("No se puede enviar.\n\nFalta:\n- "+st.block.join("\n- "));
  const now=new Date().toISOString();r.fields.labStage="Ingreso";r.fields.orderStatus="En fabricaciÃÂ³n";r.fields.sentToProductionAt=now;r.history=r.history||[];r.history.unshift({at:now,title:"Caso enviado a ASTOR LAB",detail:`${shortCode(r)} ÃÂ· ${r.fields.receptionResponsible||"RecepciÃÂ³n"}`});
  if(typeof saveRecord==="function")await saveRecord(r);else if(typeof persistActive==="function")persistActive();
  if(typeof renderAll==="function")renderAll();alert(`Caso ${shortCode(r)} enviado a producciÃÂ³n.`);render();
}
